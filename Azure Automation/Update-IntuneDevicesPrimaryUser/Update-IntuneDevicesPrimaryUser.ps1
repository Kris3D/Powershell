<#
.SYNOPSIS
    Automates the update of Intune Windows device primary users based on the last logged-on user.

.DESCRIPTION
    Authenticates via System Assigned Managed Identity.
    Fetches all company-owned Windows devices from Intune via Microsoft Graph API,
    compares the current primary user with the last logged-on user and updates
    the primary user where a discrepancy is found.
    ...
    - Sends an HTML summary email with both CSV files attached
      (importance set to High when primary user changes are detected)

    To add or remove excluded users, edit the $UsersToExclude array
...
.NOTES
    ...
    Forked from     : Shehab1Noaman - Community
                      https://github.com/Shehab1Noaman/Community/tree/main/Intune
    ...
    - Unified error handling using $_.ErrorDetails.Message ?? $_.Exception.Message
    - Removed redundant $currentUpdateHeaders — $Headers reused throughout
...
.EXAMPLE
    ...
    Schedule daily at 08:00 PM or as needed.
#>
